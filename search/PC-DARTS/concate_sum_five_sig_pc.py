...
class Cell(nn.Module):
  ...
  def forward(self, sums_prev, concats_prev, weights, outpath_weights):
    #print(f"{s0.shape};{s1.shape}")
    sums_prev = self.preprocess0(sums_prev)
    concats_prev = self.preprocess1(concats_prev)
    #print(f"{s0.shape};{s1.shape}")

    #sum:0, concat:1 within operation parameters
    concat_states = [sums_prev, concats_prev]
    sums = 0
    offset = 0
    for i in range(self._steps):
...
class Network(nn.Module):
  ...
  #model = Network(args.init_channels, CIFAR_CLASSES, args.layers, criterion)
  def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3, output_node=2, weights2_t=False):
  ...
  def forward(self, inputd,temp=1):
    sums = self.stem_0(inputd)
    concats = self.stem_1(inputd)
    for i, cell in enumerate(self.cells):
      ...
      sums, concats = cell(sums, concats, weights,outpath_weights)
    s1=torch.cat([sums,concats], dim=1)
