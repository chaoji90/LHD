...
from concate_sum_five_sig_sp import Network
...
parser.add_argument('--alphas_suffix',type=str,default='-spdarts')
...
def main():
  ...
  for epoch in range(args.epochs):
    ...
    # training
    train_acc, train_obj = train(train_queue, valid_queue, model, p_optimizer, criterion, w_optimizer, lr, scheduler, temp, C, epoch>=warmup)
...
def train(train_queue, valid_queue, model, p_optimizer, criterion, w_optimizer, lr, scheduler, temp, C, warmup):
  ...
  for step, (inputd, target) in enumerate(train_queue):
    ...
    logits = model(input_search,softmax_list[softmax_rand[step]])
