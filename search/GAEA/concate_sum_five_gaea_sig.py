...
class Network(nn.Module):
  ...
  def _initialize_alphas(self):
    ...
    #标准正态初始化arch参数
    self.alphas_normal = Variable(normalize(torch.ones(k, num_ops).cuda(), dim=-1), requires_grad=True)
    self.alphas_reduce = Variable(normalize(torch.ones(k, num_ops).cuda(), dim=-1), requires_grad=True)
    self.alphas_normal_output = Variable(normalize(torch.ones(self._output_node,self._steps).cuda(),dim=-1),requires_grad=True)
    self.alphas_reduce_output = Variable(normalize(torch.ones(self._output_node,self._steps).cuda(),dim=-1),requires_grad=True)
    #self.alphas_normal = Variable(0.01*torch.ones(k, num_ops).cuda(), requires_grad=True)
    #self.alphas_reduce = Variable(0.01*torch.ones(k, num_ops).cuda(), requires_grad=True)
    #self.alphas_normal_output = Variable(0.01*torch.ones(self._output_node,self._steps).cuda(),requires_grad=True)
    #self.alphas_reduce_output = Variable(0.01*torch.ones(self._output_node,self._steps).cuda(),requires_grad=True)
