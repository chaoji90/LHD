...
class MixedOp(nn.Module):
  ...
  def forward(self, x, weights, decay_rate=0):
    #使用softmax参数对op的输出进行加权求和
    res = sum(w * (op(x)+decay_rate*self.auxiliary_op(x)) for w, op in zip(weights, self._ops))
    #res += self.auxiliary_op(x) * decay_rate
    return res
...
class Cell(nn.Module):
  ...
  def forward(self, s0, s1, weights, outpath_weights, decay_rate=0):
    ...
    offset = 0
    for i in range(self._steps):
      s = sum(self._ops[offset+j](h, weights[offset+j], decay_rate) for j, h in enumerate(concat_states))
...
class Network(nn.Module):
  ...
  def forward(self, inputd, decay_rate=0, temp=1):
    ...
    for i, cell in enumerate(self.cells):
      ...
      s0, s1 = cell(s0, s1, weights, outpath_weights, decay_rate)
