...
from concate_sum_five_sig_surgenas import Network
from scipy.stats import entropy
from utils_minus import DecayScheduler
...
parser.add_argument('--alphas_suffix',type=str,default='-surgenas')
...
def main():
  ...
  train_queue = torch.utils.data.DataLoader( train_data, batch_size=args.batch_size, sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[:train_end]), pin_memory=True)
  ...
  beta_decay_scheduler = DecayScheduler(base_lr=args.skip_beta, T_max=args.decay_max_epoch, T_start=args.decay_start_epoch, T_stop=args.decay_stop_epoch, decay_type=args.decay)
  ...
  for epoch in range(args.epochs):
    ...
    # training
    train_acc, train_obj = train(train_queue, valid_queue, model, p_optimizer, criterion, w_optimizer, lr, scheduler, beta_decay_scheduler.decay_rate)
...
def train(train_queue, valid_queue, model, p_optimizer, criterion, w_optimizer, lr, scheduler, decay_rate):
  ...
  for step, (inputd, target) in enumerate(train_queue):
    ...
    logits = model(inputd,decay_rate)
