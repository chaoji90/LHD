...
class Network(nn.Module):
  ...
  def forward(self, inputd,temp=1):
    ...
    weights_normal = process_step_matrix(self.alphas_normal, 'dirichlet', None)
    weights_reduce = process_step_matrix(self.alphas_reduce, 'dirichlet', None)
    weights_normal_output = process_step_matrix(self.alphas_normal_output, 'dirichlet', None)
    weights_reduce_output = process_step_matrix(self.alphas_reduce_output, 'dirichlet', None)
    for i, cell in enumerate(self.cells):
      if cell.reduction:        
        weights = weights_reduce
        outpath_weights=weights_reduce_output
      else:
        weights = weights_normal
        outpath_weights=weights_normal_output
