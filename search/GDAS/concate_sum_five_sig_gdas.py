...
class MixedOp(nn.Module):
  ...
  def forward(self, x, weights):
    ...
    if weights.bool().any():
        return weights[weights.argmax()]*self._ops[weights.argmax()](x)
    else:
        return sum(weights)
...
class Network(nn.Module):
  ...
  #F.gumbel_softmax(x, tau=tau, hard=False, dim=-1)
  def gumble_max(self,weights,dim=-1):
    while True:
  ...
  def path_sample(self,reduction):
    n = 2
    start = 0
    weights_list=[]
    for i in range(self._steps):
      ...
      if reduction:
          weights_list.append(F.gumbel_softmax(self.alphas_reduce[start:end].view(-1),self.tau,True).view(n,-1))
      else:
          weights_list.append(F.gumbel_softmax(self.alphas_normal[start:end].view(-1),self.tau,True).view(n,-1))
      start = end
      n += 1
  ...
  def forward(self, inputd,temp=1):
    ...
    for i, cell in enumerate(self.cells):
      if cell.reduction: outpath_weights=torch.stack([F.gumbel_softmax(self.alphas_reduce_output[0],self.tau,True),F.sigmoid(self.alphas_reduce_output[1])])
      else: outpath_weights=torch.stack([F.gumbel_softmax(self.alphas_normal_output[0],self.tau,True),F.sigmoid(self.alphas_normal_output[1])])
      weights = self.path_sample(cell.reduction)
