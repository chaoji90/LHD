...
class Cell(nn.Module):
  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
    ...
    if reduction: self.type = 'reduction'
    else: self.type = 'normal'
...
class Network(nn.Module):
  ...
  def inputSoftmax(self,temp):
    ...
    for i in range(self._steps):
      ...
      weights_normal.append(F.softmax(self.alphas_reduce[start:end].view(-1)/temp,dim=0).view(n,-1))
      weights_reduction.append(F.softmax(self.alphas_normal[start:end].view(-1)/temp,dim=0).view(n,-1))
      start = end
      n += 1
    return {'normal':torch.cat(weights_normal),'reduction':torch.cat(weights_reduction)}
  ...
  def forward(self, inputd,temp=1):
    ...
    weights=self.inputSoftmax(temp)
    outpath_weights={'normal':torch.stack([F.softmax(self.alphas_normal_output[0]/temp,dim=-1),F.sigmoid(self.alphas_normal_output[1])]),'reduction':torch.stack([F.softmax(self.alphas_reduce_output[0]/temp,dim=-1),F.sigmoid(self.alphas_reduce_output[1])])}
    for i, cell in enumerate(self.cells):
      ...
      s0, s1 = cell(s0, s1, weights[cell.type],outpath_weights[cell.type])
  ...
  def mlc_loss(self,temp=1):
    n = 2
    start = 0
    param_logsumexp=[]
    for i in range(self._steps):
      end = start + n
      param_logsumexp.append(math.log(2/n)+torch.logsumexp(self.alphas_reduce[start:end].view(-1)/temp,dim=0))
      param_logsumexp.append(math.log(2/n)+torch.logsumexp(self.alphas_normal[start:end].view(-1)/temp,dim=0))
      start = end
      n += 1
    param_logsumexp.append(math.log(14/self._steps)+torch.logsumexp(self.alphas_normal_output,dim=-1)[0])
    param_logsumexp.append(math.log(14/self._steps)+torch.logsumexp(self.alphas_normal_output,dim=-1)[0])
    #print(param_logsumexp)
    aux_loss=torch.mean(torch.tensor(param_logsumexp))
  ...
  def _initialize_alphas(self):
    ...
    self.alphas_normal_output = Variable(1e-3*torch.randn(self._output_node,self._steps).cuda(), requires_grad=True)
    self.alphas_reduce_output = Variable(1e-3*torch.randn(self._output_node,self._steps).cuda(), requires_grad=True)
  ...
  def genotype(self,temp=1,threshold=0.4):
    ...
    with torch.no_grad():
        gene_normal = _parse(self.inputSoftmax(temp)['normal'].data.cpu().numpy())
        gene_reduce = _parse(self.inputSoftmax(temp)['reduction'].data.cpu().numpy())
