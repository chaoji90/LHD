...
from concate_sum_five_sig_beta import Network
...
parser.add_argument('--alphas_suffix',type=str,default='-beta')
...
def main():
  ...
  for epoch in range(args.epochs):
    ...
    # training
    train_acc, train_obj = train(train_queue, valid_queue, model, p_optimizer, criterion, w_optimizer, lr, scheduler, epoch, args.beta_weight)
    ...
    with torch.no_grad():
        alphas_normal=model.inputSoftmax(temp)['normal'].cpu().data
        logging.info(alphas_normal)
        alphas_reduce=model.inputSoftmax(temp)['reduction'].cpu().data
...
def train(train_queue, valid_queue, model, p_optimizer, criterion, w_optimizer, lr, scheduler, epoch, beta_weight):
  objs = utils.AvgrageMeter()
  top1 = utils.AvgrageMeter()
  top5 = utils.AvgrageMeter()

  weights = 0 + 50*epoch/100
  #weights = beta_weight*epoch/50
  logging.info('weights of mlc_loss is %f', weights)
  for step, (inputd, target) in enumerate(train_queue):
    ...
    loss = criterion(logits, target_search)+weights*model.mlc_loss()
    loss.backward()
    p_optimizer.step()
