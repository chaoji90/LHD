...
from concate_sum_five_sig_darts_minus import Network
...
parser.add_argument('--alphas_suffix',type=str,default='-darts_minus')
...
def train(train_queue, valid_queue, model, p_optimizer, criterion, w_optimizer, lr, scheduler, decay_rate):
  ...
  for step, (inputd, target) in enumerate(train_queue):
    ...
    logits = model(input_search,decay_rate)
