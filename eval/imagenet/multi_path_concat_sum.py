...
class Cell(nn.Module):
  ...
  def __init__(self, genotype, C_prev_prev, C_prev, C, reduction, reduction_prev):
    ...
    if reduction and C_prev_prev!=C:
...
class NetworkCIFAR(nn.Module):
  ...
  def forward(self, inputd):
    ...
    for i, cell in enumerate(self.cells):
      ...
      s0, s1 = cell(s0, s1, self.drop_path_prob)
...
class NetworkImageNet_lhd(nn.Module):

  def __init__(self, C, num_classes, layers, auxiliary, genotype):
    super(NetworkImageNet_20, self).__init__()
    self._layers = layers
    self._auxiliary = auxiliary
    ...
    reduction_prev = False
    self.stem1=Cell(genotype, C//2, C//2, C, True, reduction_prev)
    self.stem2=Cell(genotype, C, C*self.stem1.multiplier, C, True, reduction_prev)
    C_prev_prev, C_prev, C_curr = C, C*self.stem2.multiplier, C
    self.cells = nn.ModuleList()
    for i in range(layers):
      if i in [layers // 3, 2 * layers // 3]:
        C_curr *= 2
  ...
  def forward(self, inputd):
    ...
    s0 = self.stem0(inputd)
    s1 = s0
    s0, s1= self.stem1(s0,s1,self.drop_path_prob)
    s0, s1= self.stem2(s0,s1,self.drop_path_prob)
    for i, cell in enumerate(self.cells):
      s0, s1 = cell(s0, s1, self.drop_path_prob)
      if i == 2 * self._layers // 3:
        if self._auxiliary and self.training:
