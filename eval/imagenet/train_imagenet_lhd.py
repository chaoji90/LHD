#python train_imagenet_lhd.py --auxiliary --data /data2 --arch_file *.pl
import os
import sys
sys.path.insert(0, '../')
import numpy as np
...
from multi_path_concat_sum import NetworkImageNet_lhd as Network
...
parser.add_argument('--checkpoint', type=str, help='lr scheduler, linear or cosine')
...
if args.checkpoint:
    args.save=os.path.dirname(args.checkpoint)
else:
...
#    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.decay_period, gamma=args.gamma)
    ...
    if args.checkpoint:
        ...
        model.load_state_dict(last_info['state_dict'])
        scheduler.load_state_dict(last_info['scheduler'])
        best_acc_top1=last_info['best_acc_top1']
        best_acc_top5=last_info['best_acc_top5']
        optimizer.load_state_dict(last_info['optimizer'])
        start_epochs=last_info['epoch']
