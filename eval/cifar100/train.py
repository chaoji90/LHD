...
parser.add_argument('--dataset', type=str, default='cifar100', help='choose dataset')
...
parser.add_argument('--drop_path_prob', type=float, default=0.01, help='drop path probability')
...
def main():
  ...
  input_size, input_channels, n_classes, train_data, valid_data = utils.get_data(args.dataset, args.data, args, validation=True)
  init_channels=args.init_channels
  model = Network(init_channels, n_classes, args.layers, args.auxiliary, genotype)
  if args.model_capacity==1 and utils.count_parameters_in_MB(model)>=0.95:
      logging.info("param size = %fMB", utils.count_parameters_in_MB(model))
      logging.info("already evaluated")
      exit(0)
  while True:
      if utils.count_parameters_in_MB(model)>=args.model_capacity:
          logging.info("param size = %fMB", utils.count_parameters_in_MB(model))
          logging.info(f"init_channels: {init_channels}")
          break
      else: init_channels+=2
      model = Network(init_channels, n_classes, args.layers, args.auxiliary, genotype)
  model = model.cuda()
  args.drop_path_prob=len(genotype.normal_output['concat'])*args.drop_path_prob
  logging.info(f'drop_path_rate:{args.drop_path_prob}')
  ...
  for epoch in range(args.epochs):
    ...
    if epoch>400:
        logging.info('drop_path_prob %f', model.drop_path_prob)
        valid_acc, valid_obj = infer(valid_queue, model, criterion)
        if valid_acc > best_acc:
            best_acc = valid_acc
        logging.info('valid_acc %f, best_acc %f', valid_acc, best_acc)
        if train_acc>85: args.drop_path_prob+=0.0001
    else:
        if train_acc>75: args.drop_path_prob+=0.0005
