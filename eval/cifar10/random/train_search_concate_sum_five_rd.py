...
from concate_sum_five_sig_rd import Network
...
parser.add_argument('--alphas_suffix',type=str,default='-concate-sum-random')
parser.add_argument('--arch_file',type=str,default='new_arch')
  ...
  for epoch in range(args.epochs):
    ...
    with torch.no_grad():
        ...
        if epoch==args.epochs-1:
            pk.dump([alphas_normal.numpy(),alphas_reduce.numpy(),normal_output_weights.numpy(),reduce_output_weights.numpy()],open(args.arch_file+'.pl','wb'))
            #valid_acc, valid_obj = infer(valid_queue, model, criterion)
            #logging.info('valid_acc %f', valid_acc)
